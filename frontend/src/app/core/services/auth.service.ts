...
export class AuthService {
  private readonly tokenKey = 'burrito_token'; private readonly userKey = 'burrito_user'; private readonly currentUser = signal<User | null>(this.readUser());
  readonly user = this.currentUser.asReadonly(); readonly isAuthenticated = computed(() => !!this.token && !!this.currentUser()); readonly isAdmin = computed(() => this.currentUser()?.rol === 'admin');
  ...
  private saveSession(r: AuthResponse): void { localStorage.setItem(this.tokenKey, r.access_token); localStorage.setItem(this.userKey, JSON.stringify(r.user)); this.currentUser.set(r.user); }
